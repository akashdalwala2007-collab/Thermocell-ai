...
- **Pre-Pulse Baseline**: 1 explicit scalar voltage sample (`v_pre_pulse`, $I = 0\,\text{A}$) acquired at $t_{\text{pre}}$ immediately prior to current load application at $t = 0.0\,\text{s}$. Used strictly for `OCV` and ohmic drop calculation. It is an unloaded scalar float measurement, completely distinct from the active pulse time-series array, eliminating timestamp collisions.
