...
- **Pre-Pulse Baseline**: 1 explicit scalar voltage sample (`v_pre_pulse`, $I = 0\,\text{A}$) acquired at $t = 0.0\,\text{s}$ immediately prior to current load application. Used strictly for `OCV` and ohmic drop calculation. It is a scalar float measurement, NOT a full 8×8 thermal frame.
- **Active Pulse Window**: 100 synchronized multi-modal samples at 10Hz covering timestamps $[0.0, 9.9]\,\text{s}$ ($t_i = 0.1 \cdot i$, $i \in [0, 99]$), representing the 10.0s active pulse duration where $t = 9.9\,\text{s}$ is the final sampled active timestamp. Each sample contains $V(t)$, $I(t)$, $T_{\text{bulk}}(t)$, and an 8×8 spatial thermal frame.
- **Post-Pulse Relaxation Window**: 20 synchronized samples at 10Hz covering timestamps $[10.0, 11.9]\,\text{s}$ ($t_i = 10.0 + 0.1 \cdot i$, $i \in [0, 19]$), representing 2.0s of post-cutoff relaxation. Each sample contains $V_{\text{relax}}(t)$ and $T_{\text{bulk\_relax}}(t)$.
...
2. **`DCIR`**: Direct Current Internal Resistance derived from the immediate ohmic drop $\Delta V_0$ between `v_pre_pulse` and the first loaded active pulse sample $V(t = 0.1\,\text{s})$ under $3\,\text{A}$ discharge load ($\Omega$):
   $$\text{DCIR} = \frac{|v_{\text{pre\_pulse}} - V(t = 0.1\,\text{s})|}{I_{\text{pulse}}}$$
   *(Nominal fresh cell: $\text{DCIR} \approx 0.08 - 0.10\,\Omega \implies \Delta V_0 \approx 0.24 - 0.30\,\text{V}$ at $3\,\text{A}$. REUSE eligibility requires $\text{DCIR} \le 0.1167\,\Omega \implies \Delta V_0 \le 0.35\,\text{V}$.)*
3. **`ΔV10`**: Total voltage drop over the 10-second active pulse from pre-pulse to final active sample $t = 9.9\,\text{s}$ ($\text{V}$):
...
### Validation Protocol
- **Leave-One-Group-Out (LOGO) Cross-Validation**: Folds are partitioned strictly by physical `cell_id` across B0005, B0006, B0007, and B0018. `cycle_index` remains a feature/metadata column, ensuring all cycles for a given cell remain together.
