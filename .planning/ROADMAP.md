...
- **Outputs**: `BatteryPulseTelemetry` payload containing:
  - `v_pre_pulse`: Scalar unloaded baseline voltage in $\text{V}$ ($I = 0\,\text{A}$) captured before active pulse.
  - Active pulse: 100 synchronized samples at 10Hz covering timestamps $[0.0, 9.9]\,\text{s}$ ($t_i = \text{round}(0.1 \cdot i, 1)$) with discharge current fixed at $3.0\,\text{A}$, voltage in $\text{V}$, and bulk temperature in $^\circ\text{C}$ supporting `T_initial`, `ΔT_bulk`, and `dT/dt_max`.
  - Relaxation (`RelaxationTelemetry`): 20 synchronized samples at 10Hz covering timestamps $[10.0, 11.9]\,\text{s}$ ($t_i = \text{round}(10.0 + 0.1 \cdot i, 1)$) with voltage in $\text{V}$, bulk temperature in $^\circ\text{C}$, and current $0\,\text{A}$ ($2.0\,\text{s}$ duration).
  - All signals tagged strictly with `provenance: "SYNTHETIC"`.
...
- **Inputs**: Complete `BatteryPulseTelemetry` payload containing:
  - `v_pre_pulse`: Scalar unloaded baseline voltage in $\text{V}$ ($I = 0\,\text{A}$) captured before active pulse.
  - Active pulse: 100 synchronized samples at 10Hz covering timestamps $[0.0, 9.9]\,\text{s}$ ($t_i = \text{round}(0.1 \cdot i, 1)$) with discharge current fixed at $3.0\,\text{A}$, voltage in $\text{V}$, and bulk temperature in $^\circ\text{C}$ supporting `T_initial`, `ΔT_bulk`, and `dT/dt_max`.
  - Relaxation (`RelaxationTelemetry`): 20 synchronized samples at 10Hz covering timestamps $[10.0, 11.9]\,\text{s}$ ($t_i = \text{round}(10.0 + 0.1 \cdot i, 1)$) with voltage in $\text{V}$, bulk temperature in $^\circ\text{C}$, and current $0\,\text{A}$ ($2.0\,\text{s}$ duration).
  - Thermal: 100 corresponding 8×8 spatial thermal frames from Phase 5 in $^\circ\text{C}$ (`provenance: "SYNTHETIC"`).
